...
func _ready() -> void:
	$Blue_car/CanvasLayer/LapTimerLabel.hide()
	new_game()
	time_elapsed = 0.0
	turn = 0
...
func _process(_delta: float) -> void:
	time_elapsed += _delta
	$Blue_car/CanvasLayer/LapTimerLabel.text = "%02d:%02d:%02d" % [time_elapsed / 60, fmod(time_elapsed, 60), fmod(time_elapsed * 100, 100)]
...
func new_game():
	...
	$Blue_car.start($StartPosition.position, $desert_map/start_line.rotation + PI / 2)
